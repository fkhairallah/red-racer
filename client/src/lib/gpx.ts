...
// Share or download a file — uses the native share sheet on Android, browser download on web
export async function shareFile(content: string, filename: string, mimeType: string): Promise<void> {
  if (Capacitor.isNativePlatform()) {
    // Encode as a data URI so Capacitor Share can hand it to the OS
    const base64 = btoa(unescape(encodeURIComponent(content)));
    const dataUrl = `data:${mimeType};base64,${base64}`;
    await Share.share({ title: filename, url: dataUrl, dialogTitle: 'Share GPX file' });
  } else {
